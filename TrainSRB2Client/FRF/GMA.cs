...
namespace GMA
{
    ...
    public class Mod
    {
        ...
        public void InjectDll(String strDllName)
        {
            ...
            if (!WriteProcessMemory(pHandle, allocMem, strDllName, (UIntPtr)lenWrite, out _))
                return;
        ...
        public Task<IEnumerable<long>> AoBScan(long start, long end, string search, bool readable, bool writable, bool executable, string file = "")
        {
            return Task.Run(() =>
            {
                var memRegionList = new List<MEN>();
                       ...
                       currentBaseAddress.ToUInt64())
                {
                    ...
                    if (memRegionList.Count > 0)
                    {
                        ...
                        {
                            memRegionList[memRegionList.Count - 1] = new MEN
                            {
        ...
        private long[] CompareScan(MEN item, byte[] aobPattern, byte[] mask)
        {
